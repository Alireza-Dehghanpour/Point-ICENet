...
class FKAConvNetwork(torch.nn.Module):
    ...
    def __init__(self, in_channels, out_channels, segmentation=False, hidden=64, dropout=0.5,
        ...
        self.resnetb02 = ResidualBlock(hidden, hidden, 16, adaptive_normalization=self.adaptive_normalization)
        ...
        self.resnetb12 = ResidualBlock(2 * hidden, 2 * hidden, 16, adaptive_normalization=self.adaptive_normalization)
        self.resnetb13 = ResidualBlock(2 * hidden, 2 * hidden, 16, adaptive_normalization=self.adaptive_normalization)
        ...
        self.resnetb22 = ResidualBlock(4 * hidden, 4 * hidden, 16, adaptive_normalization=self.adaptive_normalization)
        self.resnetb23 = ResidualBlock(4 * hidden, 4 * hidden, 16, adaptive_normalization=self.adaptive_normalization)
        ...
        self.resnetb32 = ResidualBlock(8 * hidden, 8 * hidden, 16, adaptive_normalization=self.adaptive_normalization)
        self.resnetb33 = ResidualBlock(8 * hidden, 8 * hidden, 16, adaptive_normalization=self.adaptive_normalization)
        ...
        self.resnetb42 = ResidualBlock(16 * hidden, 16 * hidden, 16, adaptive_normalization=self.adaptive_normalization)
        self.resnetb43 = ResidualBlock(16 * hidden, 16 * hidden, 16, adaptive_normalization=self.adaptive_normalization)
        # --------------------------------------
    ...
    def forward(self, data, spatial_only=False, spectral_only=False, cat_in_last_layer=None):
        ...
        x0 = self.resnetb02(x0, pos, pos, data["ids00"])

        x1 = self.resnetb10(x0, pos, data["support1"], data["ids01"])
        x1 = self.resnetb11(x1, data["support1"], data["support1"], data["ids11"])
        x1 = self.resnetb12(x1, data["support1"], data["support1"], data["ids11"])
        x1 = self.resnetb13(x1, data["support1"], data["support1"], data["ids11"])

        x2 = self.resnetb20(x1, data["support1"], data["support2"], data["ids12"])
        x2 = self.resnetb21(x2, data["support2"], data["support2"], data["ids22"])
        x2 = self.resnetb22(x2, data["support2"], data["support2"], data["ids22"])
        x2 = self.resnetb23(x2, data["support2"], data["support2"], data["ids22"])

        x3 = self.resnetb30(x2, data["support2"], data["support3"], data["ids23"])
        x3 = self.resnetb31(x3, data["support3"], data["support3"], data["ids33"])
        x3 = self.resnetb32(x3, data["support3"], data["support3"], data["ids33"])
        x3 = self.resnetb33(x3, data["support3"], data["support3"], data["ids33"])

        x4 = self.resnetb40(x3, data["support3"], data["support4"], data["ids34"])
        x4 = self.resnetb41(x4, data["support4"], data["support4"], data["ids44"])
        x4 = self.resnetb42(x4, data["support4"], data["support4"], data["ids44"])
        x4 = self.resnetb43(x4, data["support4"], data["support4"], data["ids44"])
